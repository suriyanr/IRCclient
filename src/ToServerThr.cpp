...
// Thread which gets lines from the ToServer Q, and writes to Server.
// This does the slow send to server, once every 5 seconds at max.
void toServerThr(XChange *XGlobal) {
char IRC_Line[1024];
THR_EXITCODE thr_exit = 1;

   TRACE_INIT_CRASH("ToServerThr");
   TRACE();
   while (true) {
      XGlobal->IRC_ToServer.getLineAndDelete(IRC_Line);
      if (XGlobal->isIRC_QUIT()) break;
      if (XGlobal->isIRC_DisConnected()) continue;

      COUT(cout << "toServerThr: " << IRC_Line << endl;)
      if (strlen(IRC_Line) > 1) {
         XGlobal->putLineIRC_Conn(IRC_Line);
         sleep(3);
      }
   }
   COUT(cout << "toServerThr: Quitting" << endl;)
